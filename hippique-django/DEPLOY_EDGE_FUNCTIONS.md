...
2. **Créer la première fonction : turboquinte-filter**
   - Cliquez sur "New Function"
   - Nom : `turboquinte-filter`
   - Vérifiez que l'URL sera : `https://qfkyzljqykymahlpmdnu.supabase.co/functions/v1/turboquinte-filter`
   - Copiez le code ci-dessous dans l'éditeur

3. **Code pour turboquinte-filter**
...
4. **Créer la deuxième fonction : turboquinte-backtest**
   - Cliquez sur "New Function"
   - Nom : `turboquinte-backtest`
   - Vérifiez que l'URL sera : `https://qfkyzljqykymahlpmdnu.supabase.co/functions/v1/turboquinte-backtest`
   - Copiez le code depuis `C:\Users\HP 360\Desktop\hippique-django\supabase\functions\turboquinte-backtest\index.ts`

5. **Vérifier le déploiement**
   - Une fois déployées, les URLs doivent être accessibles :
...
6. **Tester le filtre**
   - Allez sur http://72.62.181.239:8090/
   - Les filtres doivent maintenant fonctionner !

## ✅ Confirmation

Une fois les Edge Functions déployées, le frontend sur le port 8090 sera pleinement fonctionnel.
