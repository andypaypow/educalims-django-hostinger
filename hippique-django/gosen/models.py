...
class UserProfile(models.Model):
    ...
    def filtres_gratuits_restants(self):
        if self.date_reset_filtres != timezone.now().date():
            self.filtres_gratuits_utilises = 0
            self.date_reset_filtres = timezone.now().date()
            self.save(update_fields=['filtres_gratuits_utilises', 'date_reset_filtres'])
    ...
    def activer_abonnement(self, type_abonnement, duree_jours=None):
        ...
        if type_abonnement == 'mensuel':
            self.date_fin_abonnement = maintenant + timezone.timedelta(days=30)
        elif type_abonnement == 'annuel':
            self.date_fin_abonnement = maintenant + timezone.timedelta(days=365)
        ...
        if self.date_debut_abonnement and self.date_fin_abonnement and self.est_abonne:
            ...
            if type_abonnement == 'mensuel':
                self.date_fin_abonnement = ancienne_fin + timezone.timedelta(days=30)
            elif type_abonnement == 'annuel':
                self.date_fin_abonnement = ancienne_fin + timezone.timedelta(days=365)
